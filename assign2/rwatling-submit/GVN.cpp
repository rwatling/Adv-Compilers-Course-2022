...
//Option for svn
bool SVNEnabled;
static cl::opt<bool, true> GVNEnableSVNOnly("svn", cl::desc("Enable superlocal value numbering (no gvn)"), cl::location(SVNEnabled));

//Option of lvn
bool LVNEnabled;
static cl::opt<bool, true> GVNEnableLVNOnly("lvn", cl::desc("Enable local value numbering (no gvn)"), cl::location(LVNEnabled));
...
// is SVN enabled
bool GVNPass::isGVNSVNOnlyEnabled() const {
  return Options.AllowSVNOnly.getValueOr(GVNEnableSVNOnly);
}

// is LVN enabled
bool GVNPass::isGVNLVNOnlyEnabled() const {
  return Options.AllowLVNOnly.getValueOr(GVNEnableLVNOnly);
}
...
bool GVNPass::runImpl(Function &F, AssumptionCache &RunAC, DominatorTree &RunDT,
  ...
  //errs() << "Hello\n";
  //errs() << "svn enabled:" << SVNEnabled <<"\n";
  //errs() << "lvn enabled:" << LVNEnabled << "\n";
...
bool GVNPass::processBlock(BasicBlock *BB) {
  ...
  //Cleanup all Value Numbering information that is not relevant to this block
  if(LVNEnabled) {
      cleanupGlobalSets();
  }
...
bool GVNPass::iterateOnFunction(Function &F) {
  ...
  if (!SVNEnabled) {
    ...
    //Attempted this with vectors but the referencing got messy
    BasicBlock* EBBList[256][256]; 
    int row = 0;
    int col = 0;

    //Null out all entries. Ran into some weird issues without this.
    for (int i = 0; i < 256; i++) {
      for (int j =0; j < 256; j++) {
        EBBList[i][j] = NULL;
      }
    ...
    // Create EBB list based on predecessor criteria
    for (BasicBlock &BB : F) {
      if (pred_size(&BB) > 1) {        
        col = 0;
        EBBList[++row][col++] = &BB;
    ...
    // Print and process extended basic blocks
    for (int i = 0; i <= row; i++) {
      errs() << "{";
      for (int j = 0; j < 256; j++) {
        if (EBBList[i][j]) {
          ...
          errs() << EBBList[i][j]->getName();
          Changed |= processBlock(EBBList[i][j]);
        }
      }

      cleanupGlobalSets(); //Do not need to pop VN, uses DT
      errs() << "}\n";
    }
