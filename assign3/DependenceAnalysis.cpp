...
bool DependenceInfo::testSIV(const SCEV *Src, const SCEV *Dst, unsigned &Level,
  ...
  if (SrcAddRec && DstAddRec) {
    ...
    //errs() << "SrcCoeff=" << *SrcCoeff <<"\n";
   // errs() << "DstCoeff=" << *DstCoeff <<"\n";
...
DependenceInfo::depends(Instruction *Src, Instruction *Dst,
  ...
  */
  ...
  //Try to use GEP to find subscript
  ...
  else { // this else section get moved out if not using GEP
    LLVM_DEBUG(dbgs() << "    ignoring GEPs\n"); 
    //Delinearize to find subscript pairs
    const SCEV *SrcSCEV = SE->getSCEV(SrcPtr);
    const SCEV *DstSCEV = SE->getSCEV(DstPtr);
    LLVM_DEBUG(dbgs() << "    SrcSCEV = " << *SrcSCEV << "\n");
    LLVM_DEBUG(dbgs() << "    DstSCEV = " << *DstSCEV << "\n");
  ...
  for (unsigned P = 0; P < Pairs; ++P) {
    ...
    // Only need to consider innermost subscript
    if ((P == (Pairs - 1)) && !PossiblyLoopIndependent) {      
      // Get step recurrence from source
      const SCEVAddRecExpr *SrcAddRec = dyn_cast<SCEVAddRecExpr>(Pair[P].Src);
      const SCEV *SrcCoeff = SrcAddRec->getStepRecurrence(*SE);

      //Get the result element type for load/store
      Type *ResType = SrcGEP->getResultElementType();
      size_t element_size = 0;

      if (ResType->isIntegerTy()) {
        element_size = sizeof(int);     
      } else if (ResType->isFloatingPointTy()) {
        element_size = sizeof(double);
      } else  {
        errs() << "? type\n";
      }

      // Convert step recurrence to usable integer and update it according to cache information
      Value *myVal = cast<SCEVConstant>(SrcCoeff)->getValue();
      ConstantInt *CI = dyn_cast<ConstantInt>(myVal);
      uint64_t strideInt = CI->getZExtValue();

      strideInt = strideInt / (element_size / WORD_SIZE);

      if (strideInt < (CACHE_LINE_SIZE / element_size)) {
        errs() << *Src << "\n";
        errs() << "level: " << CommonLevels << "\n";
        errs() << "stride: " << strideInt <<"\n";
      }
